...
Create a file called `/config/custom_intents/default.yaml` and move everything that's under `intents:` in `configuration.yaml`, but unindent the resulting dictionary so that it's on the first level in the file.
...
Next, edit `configuration.yaml` and alter the `intents:` key

```yaml
# /config/configuration.yaml
intents: !include_dir_merge_named custom_intents
```
...
To add custom intents, add a filed called `custom_sentences/xx/intent_handler.yaml`, where:
